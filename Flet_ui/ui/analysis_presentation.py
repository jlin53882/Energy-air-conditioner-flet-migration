...
ANALYSIS_PRESENTATION: dict[str, AnalysisPresentation] = {
    ...
    "airside.humidification": AnalysisPresentation(
        "加濕水量／蒸汽需求估算：以入口與設計目標的濕度比差求加濕水量，並以當地大氣壓力的水蒸發潛熱估算"
        "蒸汽加濕熱量（下限，不含給水預熱與設備損失）。設計目標只用來取得目標濕度比，不代表實際蒸汽加濕的出口狀態或過程。",
        "m_w = ṁ(W_target − W_in);  Q_steam = m_w · h_fg",
        "加濕負荷",
        key_metrics=("加濕水量", "蒸汽加濕熱量", "入口濕度比", "目標濕度比"),
        chart_title="濕空氣線圖（入口與設計目標）",
    ),
    "airside.airflow_capacity": AnalysisPresentation(
        "標準空氣快算：以固定 ρ、cp 由風量與溫差求顯熱容量（大小，不分冷卻或加熱）或反算風量。狀態精算：以進出風狀態的焓差"
        "求全熱／顯熱／潛熱（正值為冷卻、負值為加熱），或由全熱容量（取大小）反算風量（以進風狀態比容換算）。",
