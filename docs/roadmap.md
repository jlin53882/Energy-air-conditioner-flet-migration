...
**結果**：`domain/batch.py`（掃描、類別維度比較、單因子敏感度；逐點失敗記錄）與
`application/batch.BatchService`（冷凍循環與冷凝器 Exergy，指標只取與 reference state 無關的量）。
UI 新增「批次與比較」路由：參數掃描（一或兩個變數，曲線圖）、冷媒比較（冷凍循環；長條圖或
每種冷媒一條曲線）與敏感度分析（龍捲風圖）。契約見 `domain-contracts.md` §13。
