...
Flet `ResponsiveRow` 的斷點依頁面寬度判斷，而不是依父容器寬度。放在半寬欄位中的指標卡片因此最多每列兩張；情境面板改為浮動抽屜，也是為了讓頁面寬度斷點與實際可用寬度一致。`DedicatedAnalysisView` 的 `column_split` 決定寬版輸入欄與結果欄的欄寬（預設 6／6）：濕空氣性質為 4／8、濕空氣線圖為 5／7；濕空氣性質寬版時焓濕圖與完整性質表並排、關鍵數值一列四張。
