...
- 狀態庫路由（`StateLibraryView`，工作區分區）：列出已保存的冷媒與濕空氣狀態點（改名、複製、刪除；刪除需再按一次「確認刪除」），並比較 A 與 B（B − A）。比較表的數值依全域輸出單位換算，溫度差以溫差（`DeltaT`）換算，避免英制加上零點偏移；焓、熵基準不同時不相減並說明原因。狀態庫透過 `StateLibraryService.add_listener` 在其他頁面保存後立即更新。
- 分析定義可提供 `state_points`：無參數函式，回傳最近一次成功計算可保存的 domain 狀態點。`DedicatedAnalysisView.attach_state_library()` 連接狀態庫後，結果區動作列顯示「儲存狀態點」選單（`StateSaveMenu`：逐一保存或「全部儲存」，一律經 `StateLibraryService.save_many` 全有或全無地保存，並顯示成功或失敗原因）；只有成功狀態才列出，結果失效、錯誤或切換分析時隱藏，避免保存舊結果。目前提供狀態點的有：飽和性質（泡點、露點）、過熱／過冷（露點、泡點、量測點）、冷凍循環（狀態 1、2s、2、3、4）、冷凝器 Exergy（入口、出口）與濕空氣性質（濕空氣狀態）；狀態查詢頁以同一個選單保存查詢結果（記錄查詢時實際使用的 reference state，水的理想氣體模式標記為理想氣體）。選單只保存 domain 回傳的狀態點，不從格式化文字反推數值。
- 分析定義可提供 `structured_result`：無參數函式，回傳最近一次成功計算的 `StructuredResult`（`Flet_ui/ui/structured_result.py`：`key_metrics` 關鍵數值、`groups` 完整性質表、`process_groups` 計算過程、圖表標題與圖例）。資料類別只存放已格式化的文字，不依賴 Flet；結果區只負責排版，不重新計算或換算。目前 `PsyModule`（濕空氣性質兩種模式）、`SaturationModule` 與 `SuperheatSubcoolingModule` 提供原生 `structured_result`；其他分析（壓縮機、蒸發器、冷凝器含 Exergy、冷凍循環、空氣處理、濕空氣線圖）未提供時由 `structured_from_text()` 轉換模組的格式化文字。`structured_from_text()` 只供顯示：它讀取的是已四捨五入、已換成輸出單位的文字，不得作為狀態點、批次計算或任何後續計算的資料來源；需要數值的功能必須改由 domain／application 服務的結構化結果取得。轉換規則：關鍵數值依 `analysis_presentation` 中該分析宣告的 `key_metrics`（結果名稱，依序挑選、找不到的略過；未宣告時取前四個）挑選，完整性質表列出全部結果（所有結果都已是關鍵數值時省略），圖表卡標題取自 `chart_title`。新增分析時應在說明表宣告 `key_metrics`，測試會檢查宣告的名稱確實出現在預設輸入的計算結果中。提供原生 `structured_result` 的分析自行決定關鍵數值（例如過熱／過冷的第一個關鍵數值名稱隨判讀結果變動，飽和性質在已知溫度時只有三個），`key_metrics` 對它們不生效，因此不得宣告，測試會檢查。濕空氣性質的兩種模式由 `PsychrometricResultBuilder` 產生：四個關鍵數值（RH、露點、焓、濕度比）、帶濕球／露點輔助線的焓濕圖、分成「輸入值／計算結果」的完整性質表，以及飽和壓力、飽和濕度比等中間值；海拔欄位下方即時顯示推算的大氣壓力。結構化結果與文字結果必須來自同一份服務回傳的狀態。關鍵數值與性質表的數值欄使用等寬數字字型（`TOKENS.mono_font`，不含中文字形），只放數值、單位與代碼（例如冷媒名稱、`ASHRAE`）；判讀狀態等中文說明放在名稱欄，公式提示同樣只用 ASCII 與數學符號。
...
- 空調負荷路由（`AirLoadView` + `AirLoadModule`，空氣處理分區）：新風負荷、加濕負荷（加濕水量／蒸汽需求估算）與風量／冷量換算，計算委派給 `AirLoadService`。新風負荷在濕空氣線圖上以連線表示外氣到室內；加濕負荷只標示入口與設計目標兩點、不畫連線——設計目標只用來取得目標濕度比，不代表實際蒸汽加濕過程的出口或軌跡。容量欄位標示為大小：快算「顯熱容量（大小）」、精算反算「全熱容量（取大小）」；精算結果的全熱／顯熱／潛熱保留符號（正值為冷卻、負值為加熱）。風量／冷量換算以「計算方式（快算：標準空氣；精算：濕空氣狀態）」與「已知條件（風量或冷量）」兩個分段按鈕切換輸入列，兩者都是語意輸入，切換後結果失效。三項分析都提供 `state_points`（外氣／室內、加濕前／目標、進風／出風的 `AirStatePoint`），可保存到狀態庫；標準空氣快算沒有空氣狀態，不提供狀態點。
- 批次與比較路由（`BatchView` + `BatchModule`，冷凍系統分區）：參數掃描（一或兩個輸入，第二個輸入的每個數值一條曲線）、冷媒比較（冷凍循環；可另掃描一個輸入，每種冷媒一條曲線，不掃描時畫長條圖）與敏感度分析（龍捲風圖）。計算委派給 `BatchService`，只使用冷凍循環與冷凝器 Exergy 的結構化結果，不從結果文字反推數值。計算對象（冷凍循環／冷凝器）、掃描變數、T_b 設定與輸出指標都是語意輸入，切換後結果失效；切換計算對象或 T_b 設定時，仍有效的掃描變數與使用者輸入的範圍保留，失效時才改為預設變數與預設範圍。批次指標與 reference state 無關，因此不提供 Reference State 選單。無法計算的點不中斷整批計算，列在「無法計算的點」並在曲線上斷線。圖表使用 `FigurePanel`（Matplotlib），以 constrained layout 配合面板縮放；`FigurePanel` 匯入時以 `use_cjk_fallback_fonts()` 把已安裝的中文字型接在 Matplotlib `font.family` 之後作為逐字備援，圖表中的中文不會變成方框（同時適用於 P-h／T-s 圖）。
- 冷凍循環路由（`RefrigerationCycleView` + `RefrigerationCycleModule`）提供蒸氣壓縮循環（P-h 圖沿用 `generate_thermo_diagram`，並使用循環結果 `reference_state` 攜帶的、求解時實際使用的 policy，不重新解析 Auto）。
- 飽和性質路由（`SaturationView` + `SaturationModule`，分析 `refrigerant.saturation`）：已知壓力或已知溫度（分段按鈕，屬於語意輸入），查詢泡點與露點狀態點；已知溫度時隱藏壓力類型、壓力、海拔與大氣壓力列。過熱／過冷路由（`SuperheatSubcoolingView` + `SuperheatSubcoolingModule`，分析 `refrigerant.superheat_subcooling`，原本位於冷凍循環頁）：現場量測壓力與管溫判讀過熱度或過冷度。兩頁的壓力在錶壓模式使用錶壓單位（`kPag`、`psig`…）並加上大氣壓力（預設 101.325 kPa，可選填海拔自動計算），在絕對壓力模式使用絕對單位；切換模式時換算數值以維持相同的實際壓力（契約見 `docs/hvac-ui-domain-contract.md`）。兩頁都以原生 `structured_result` 呈現：關鍵數值與輸入／狀態點性質表（T、P、h、s、ρ、v，`thermo_state_presentation.py`）直接取自 domain 回傳的 `ThermoStatePoint`，與文字結果使用相同單位與位數。冷媒欄位使用 `BaseAnalysisModule.create_fluid_row`，下方附常用冷媒快捷（`COMMON_REFRIGERANTS`，皆為 CoolProp 可直接計算的名稱）；點選等同修改冷媒並使結果失效。
- 蒸氣壓縮循環、冷凝器 Exergy、飽和性質與過熱／過冷各自提供 Reference State 選單（`BaseAnalysisModule.create_reference_state_row`，預設 Auto），不跟隨物性查詢頁。
