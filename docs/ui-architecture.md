...
- 空調負荷路由（`AirLoadView` + `AirLoadModule`，空氣處理分區）：新風負荷、加濕負荷（水量與蒸汽熱量）與風量／冷量換算，計算委派給 `AirLoadService`。新風與加濕過程標示在濕空氣線圖上。風量／冷量換算以「計算方式（快算：標準空氣；精算：濕空氣狀態）」與「已知條件（風量或冷量）」兩個分段按鈕切換輸入列，兩者都是語意輸入，切換後結果失效。三項分析都提供 `state_points`（外氣／室內、加濕前／目標、進風／出風的 `AirStatePoint`），可保存到狀態庫；標準空氣快算沒有空氣狀態，不提供狀態點。
