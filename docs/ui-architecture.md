...
成功計算快照分別保存標準 SI 結果、質量、中繼資料、當次輸入摘要及目前輸出單位下的格式化結果；`ResultPanel` 只呈現結構化指標與中繼資料，摘要文字、原始詳細文字與複製文字則是彼此獨立的呈現投影。呈現控制項是計算快照的投影，不得反過來充當快照本身；摘要與複製內容須由保存的快照組成，詳細文字只包含格式化性質輸出。輸出單位切換時，從同一份 SI 快照更新指標、詳細文字及完整摘要，不得重新查詢；語意輸入失效或重設時，必須一併清除摘要、格式化文字與複製能力。
