...
- 已知低風險相容耦合：`ThermoDiagramView.perform_calculation()` 目前直接呼叫 `module._on_plot_click(event)`（private method）。這是既有 `ThermoDiagramModule` 尚未提供對應 public API 造成的邊界瑕疵；風險低（僅限 `ThermoDiagramView` 這一個呼叫點），暫不視為 blocker，後續若 `ThermoDiagramModule` 補上 public `plot()`／`perform_plot()` 等方法，應同步移除這個直接依賴。
