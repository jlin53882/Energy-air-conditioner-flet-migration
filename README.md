...
- **冷凍系統**：壓縮機（壓縮比、功、等熵／容積效率、可逆功、㶲分析）、蒸發器、冷凝器（放熱率、能量／熵／Exergy 平衡），蒸氣壓縮循環（COP、流量、功率與 P-h 圖）、冷媒飽和性質（泡點、露點、溫度滑移；已知壓力時另提供蒸發潛熱），以及現場過熱度／過冷度判讀（錶壓輸入、常用冷媒快捷）。「批次與比較」頁對冷凍循環與冷凝器 Exergy 做參數掃描（一或兩個變數的曲線）、冷媒比較與敏感度分析（龍捲風圖）。
- **空氣處理**：濕空氣性質、兩股氣流混合、顯熱加熱／冷卻、冷卻除濕盤管負荷（顯熱比、冷凝水）與送風量估算，過程標示在濕空氣線圖上；空調負荷頁提供新風負荷（顯熱／潛熱）、加濕水量／蒸汽需求估算（依設計目標濕度比，非加濕過程模擬），以及風量與冷量換算（標準空氣快算或濕空氣狀態精算）。
- **圖表**：P-h、T-s 熱力圖與可標示多點的濕空氣線圖。
- **工具**：含 RT、kcal/h、CMH／CFM 與溫差的單位換算。
- **狀態庫**：保存冷媒與濕空氣狀態點（改名、複製、刪除），並比較兩個狀態（B − A）；資料存在使用者家目錄的 `.hvac_workspace/`。
